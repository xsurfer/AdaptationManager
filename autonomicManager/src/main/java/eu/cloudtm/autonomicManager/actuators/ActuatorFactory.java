...
public class ActuatorFactory {
   ...
   private IActuator createCloudTM() {
      ...
      if (isRadargun) {
         log.info("Creating CloudTM actuator plus Radargun...");
         ...
         log.info("Creating CloudTM actuator (without Radargun)...");
   ...
   private IActuator createFutureGrid() {
      if (isRadargun) {
         log.info("Creating FutureGrid actuator plus Radargun...");
         return new FutureGridActuator(createRadargunClient(), jmxPort, domain, cacheName, availableMachines(), initMachinesUp()); // with radargun
      } else {
         log.info("Creating FutureGrid actuator (without Radargun)...");
         return new FutureGridActuator(jmxPort, domain, cacheName, availableMachines(), initMachinesUp());
      }
   }
